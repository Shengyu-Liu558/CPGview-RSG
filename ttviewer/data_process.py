...
for f in rec.features:
    ...
    if f.type != 'source' and f.type.lower() == 'cds' and 'gene' in f.qualifiers:
        if "join" in str(f.location) and "('gene'," in str(f.qualifiers):
            ...
            y_nums += 1
            location1 = str(f.location).replace("<", "").replace(">", "").replace("join{", "").replace("}", "").replace(
...
if loca11 == [] and loca12 == [] and loca13 == [] and loca14 == []:
    new_raw_list0 = []
    new_raw_list1 = []
    new_raw_list2 = []
...
if not folder1 and not folder2:
    ...
    if new_raw_list0 != [] and new_raw_list1 != [] and new_raw_list2 != []:
        ...
        csvfile4.close()
    elif new_raw0 != [] and new_trans0 != []:
        name5 = ab_path2 + "/trans_splicing_gene.csv"
        name6 = ab_path2 + "/trans_splicing_subgene.csv"

        csvfile5 = open(name5, 'w', newline='', encoding="utf-8")
        writer5 = csv.writer(csvfile5)
        writer5.writerow(
            ["Gene", "Exon", "start", "end", "strand", "direction", "label_from", "label_to", "label_label"])
        writer5.writerows(new_raw0)
        writer5.writerows(new_trans0)

        csvfile5.close()

        csvfile6 = open(name6, 'w', newline='', encoding="utf-8")
        writer6 = csv.writer(csvfile6)
        writer6.writerow(
            ["Gene", "Exon", "start", "end", "strand", "direction", "label_from", "label_to", "label_label"])
        writer6.writerows(new_trans0)
        csvfile6.close()
    else:
    ...
    print(
        "---  New folder ", file1, " already exists here. Please delete it "
                                   "or them first!  ---")
